...
namespace mrs_octomap_tools
{
  namespace octomap_rviz_visualizer
  {
    ...
    template <typename OcTree_t>
    OctomapRvizVisualizer<OcTree_t>::OctomapRvizVisualizer(const rclcpp::NodeOptions& options) : rclcpp::Node("octomap_rviz_visualizer", options)
    {
      ...
      if (_use_colored_map_)
      {
        ...
        {
          RCLCPP_WARN(this->get_logger(), "[%s]: Colored map requested, but this node does not support colors. Change octree_type in launch file.",
                      this->get_name());
        }
      }
